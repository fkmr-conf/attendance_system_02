...
############################################################################################################################
    ...
    def _make_list_of_dic(self, request):#↑と一本化するか？
...
#細かく関数にしたので、Employer_function()._approve_reportにも適用できるようにする
    ...
    def _create_year_month_choice(self, request):
        #dateを基準に最古と最新のworking reportを取得
        first=working_record.objects.filter(employee_ID=request.session['ID']).aggregate(Min('date'))['date__min']
        latest=working_record.objects.filter(employee_ID=request.session['ID']).aggregate(Max('date'))['date__max']
        #3
        year_month_list=[]
        the_date=first
        while True:
            year_month=str(the_date.year) + " / " + str(the_date.month)
    ...
    def _check_my_working_report(self, request):
        ...
        year_month_choice=self._create_year_month_choice(request)
        #選択肢を作成
        form=select_year_month_form()
        form.fields['year_and_month'].choices=year_month_choice        

        if request.method=='POST':
            the_basis_date=request.POST['year_and_month']#ついたちが取得される
        else:
